...
namespace Module.ColorfulHierarchy_main.Colorful_Hierarchy.Editor
{
    [InitializeOnLoad]
    public class ColorfulHierarchyManager
    {
        ...
        private static void HierarchyHighlight_OnGUI(int inSelectionID, Rect inSelectionRect)
        {
            ...
            if (GO_Label != null)
            {
                Module.ColorfulHierarchy_main.Colorful_Hierarchy.ColorfulHierarchy Label = GO_Label.GetComponent<Module.ColorfulHierarchy_main.Colorful_Hierarchy.ColorfulHierarchy>();
                ...
                if (Label != null && Event.current.type == EventType.Repaint)
                {
                    ...
                    {
                        if (BKCol != Module.ColorfulHierarchy_main.Colorful_Hierarchy.ColorfulHierarchy.DEFAULT_BACKGROUND_COLOR) // Reduce opacity 
                    ...
                    if (BKCol.a > 0f)
                    {
                        ...
                        {
                            EditorGUI.DrawRect(BackgroundOffset, Module.ColorfulHierarchy_main.Colorful_Hierarchy.ColorfulHierarchy.DEFAULT_BACKGROUND_COLOR);
                        }
                        if (ObjectIsSelected)
